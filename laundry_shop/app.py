...
def api_income_by_month():
    try:
        # read from Income persistence table
        incomes = Income.query.order_by(Income.date.desc()).all()

        # build monthly and daily aggregates
        ...
        for inc in incomes:
            overall_total += float(inc.total or 0.0)
            month_label = f"{calendar.month_name[inc.date.month]} {inc.date.year}"
            monthly[month_label] += float(inc.total or 0.0)
            # use readable day label
            day_label = inc.date.strftime("%B %d, %Y")
            daily[day_label] += float(inc.total or 0.0)

        # sort by date descending for UI friendliness (latest first)
        def month_sort_key(m):
            # convert "MonthName YYYY" back to (year, month) for sorting
            parts = m.split()
...
def delete_income_month():
    try:
        ...
        # delete Income rows matching that month/year
        deleted = 0
        rows = Income.query.filter(db.extract('year', Income.date) == year, db.extract('month', Income.date) == month_num).all()
        for r in rows:
            db.session.delete(r)
            deleted += 1
        db.session.commit()
...
def delete_income_day():
    try:
        ...
        target_date = _dt.strptime(day_label, "%B %d, %Y").date()
